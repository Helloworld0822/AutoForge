use crate::clients::openrouter::{
    AiMessage, AiProvider, AiRequest, AiResponse, AiRole, OpenRouterClient, ResponseFormat,
};
...
pub fn parse_json<T: for<'de> Deserialize<'de>>(content: &str) -> Result<T> {
    ...
    serde_json::from_str(candidate)
        .map_err(|error| AutoForgeError::OpenRouter(format!("invalid structured output: {error}")))
}

pub fn project_spec_system() -> &'static str {
    "Extract requirements without summarizing or inventing facts. Return only the requested JSON schema. Preserve source references and record uncertainty in unknowns or contradictions."
}
